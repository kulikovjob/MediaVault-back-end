...
import { getDatabaseInstance, getDatabaseConnectionUrl } from '../utils/databaseUtils';
...
//     }
//
//     const connectionUrl = `postgres://${username}:${password}@${process.env.DATABASE_HOST}:${process.env.DATABASE_PORT}/${process.env.DATABASE_NAME}`;
//
//     // Проверка правильности URL
//     if (!connectionUrl) {
//       return res.status(500).json({ message: 'Failed to create connection URL' });
//     }
//
//     // Вызов getDatabaseInstance и передача connectionUrl
//     const dbInstance = getDatabaseInstance(connectionUrl);
//
//     res.status(200).json({ message: 'Authentication successful', dbInstance });
//   } catch (error) {
